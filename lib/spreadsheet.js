...
class Spreadsheet {
   ...
   * @param {string[]} [params.scope=['https://spreadsheets.google.com/feeds']]
   */
  constructor({ client_id, client_secret, spread_sheet_id, redirect_url, port, token, scope }) {
   ...
   * @param {number} [line=1]
   * @param {number} [firstData=3]
   * @param {boolean} [vartical=false] TODO
   * @returns {Spreadsheet}
   */
  defineTitle(line = 1, firstData = 3, vartical = false) {
    Sheet.defineTitle(line, firstData, vartical);
    return this;
  }
   ...
   * @param {Function} [formatter] - will be executed after getting sheet, even async function will work as well.
   * @param {boolean} [object=true]
   * @returns {Object} if json is true, return json, otherwise a sheet instance.
   */
  async getSheet(sheetName, range, formatter, object = true) {
    if (!range) {
    ...
    const sheet = new Sheet(sheetInfo, formatter);
   ...
   * @param {string[]} [ranges]
   * @param {Function[]} [formatters] - will be executed after getting sheet, even async function will work as well.
   * @param {boolean} [object=true]
   * @returns {Object}
   */
  async getSheetMap(names, ranges = [], formatters = [], object = true) {
    const sheetInfo = await this.getSheetInfo();
    ...
    return Aigle.transform(names, async (map, name, i) => {
      ...
      map[name] = await this.getSheet(name, ranges[i], formatters[i], object);
      Logger.debug('getSheetMap', `finished. ${name}:${Date.now() - start}`);
    }, {});
  }
}
