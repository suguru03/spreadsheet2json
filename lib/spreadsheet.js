...
class Spreadsheet {
  ...
  async getSheetInfo(opts) {
    if (opts) {
      this.sheetInfo = await sheets.spreadsheets.getAsync(Object.assign({
        auth: this.auth,
  ...
  async getRange(sheetName, start = 'A1', end) {
    ...
    return `${sheetName}!${start}:${convertNumberToSheetTitle(columnCount)}${rowCount}`;
  }
  ...
  async getSheetMap(names, { filter, validator, formatter, filters = [], validators = [], formatters = [], ranges, object = true } = {}) {
    ...
    return _.transform(valueRanges, (map, info, i) => {
      const name = names[i];
      const opts = {
